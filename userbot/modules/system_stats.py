...
async def amireallyalive(alive):
    ...
    output = (
        f"❖ **[Wiki-Userbot](https://github.com/Wiki28/Wiki-Userbot)** __is Up and Running.__\n"
        f"╭┈────────────••───────━\n"
        ...
        f"╰┈───────••────────────━\n"
        f"       [𝗦𝘂𝗽𝗽𝗼𝗿𝘁](https://t.me/WikiTapiGroup) | [𝗖𝗵𝗮𝗻𝗻𝗲𝗹](https://t.me/WikiTapiChannel) | [𝗢𝘄𝗻𝗲𝗿](https://t.me/Hanya_W)"
    )
    if ALIVE_LOGO:
        try:
            logo = ALIVE_LOGO
...
async def amireallyalive(alive):
    ...
    output = (
        f"   **Userbot powered by Wiki W** \n\n"
        f"❒ **ᴏᴡɴᴇʀ   :** [Owner](https://t.me/Hanya_W)    \n"
        f"❒ **ꜱʏꜱᴛᴇᴍ  :** `Android 10`   \n"
        f"❒ **ᴛᴇʟᴇᴛʜᴏɴ :** `v{version.__version__}`   \n"
        f"❒ **ᴘʏᴛʜᴏɴ  :** `v{python_version()}` ㅤ\n"
        f"❒ **ʙᴏᴛ ᴠᴇʀ :** `v{BOT_VER}`           \n"
        f"❒ **ᴍᴏᴅᴜʟᴇ  :** `{len(modules)}` ㅤ\n\n"
        f"   [𝗥𝗘𝗣𝗢](https://github.com/Wiki28/Wiki-Userbot) | [𝗦𝗨𝗣𝗣𝗢𝗥𝗧](t.me/WikiTapiGroup) | [𝗜𝗡𝗦𝗧𝗔𝗚𝗥𝗔𝗠](Instagram.com/terserah_wiki)"
    )
    if ALIVE_LOGO:
        try:
            logo = ALIVE_LOGO
